...
export async function GET(request: Request, context: Context) {
  ...
  try { const { id } = await context.params; return NextResponse.json({ data: { snapshot: await evaluateSharedHealthForUser(session.user.id, id) } }, { headers: { "Cache-Control": "no-store" } }); }
