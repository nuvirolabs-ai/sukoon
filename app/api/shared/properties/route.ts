...
import { listSharedPropertiesForUser, SharingInputError } from "@/lib/sharing";
...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json({ data: { properties: await listSharedPropertiesForUser(session.user.id) } }, { headers: { "Cache-Control": "no-store" } }); }
