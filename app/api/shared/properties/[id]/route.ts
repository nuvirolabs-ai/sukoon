...
import { getSharedPropertyForUser, SharingInputError } from "@/lib/sharing";
...
export async function GET(request: Request, context: Context) {
  ...
  try { const { id } = await context.params; return NextResponse.json({ data: await getSharedPropertyForUser(session.user.id, id) }, { headers: { "Cache-Control": "no-store" } }); }
