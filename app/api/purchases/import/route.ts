...
/**
 * T02 explicit import executor. Copies the caller's own candidate evidence
 * into the caller's owned Passport with source lineage and a fresh scan.
 * Requires explicit confirmation; creates no grants, transfers no ownership.
 */
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ data: await executePurchaseImport(principal, input as Record<string, unknown>) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
