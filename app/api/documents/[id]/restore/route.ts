...
import { restoreDocumentForUser, VaultInputError } from "@/lib/vault-repository";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { return NextResponse.json({ data: await restoreDocumentForUser(session.user.id, id) }); }
