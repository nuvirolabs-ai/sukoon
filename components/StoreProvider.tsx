...
import { clientReviewClient, getServerSnapshot, getSnapshot, persistState, replaceState, requestOtp, retrySession, signInClientReview, signOut, startClientStore, subscribe, verifyOtp } from "@/lib/client-store";
    ...
    <main className="native-safe-screen mx-auto min-h-dvh max-w-[430px] bg-white px-6 pb-10 pt-24 text-foreground">
      <div className="mx-auto max-w-sm">
        ...
        <section className="mt-12 rounded-[26px] border border-line bg-[#fbfaf7] p-5 shadow-soft">
          ...
          <p className="mt-1 text-[13px] leading-5 text-ink-muted">{clientReviewClient ? "Use your review access code to open the synthetic client review account." : "Use an email OTP to open a separate account. No password or sample property data is created."}</p>
          <form className="mt-5 space-y-3" onSubmit={(event) => { event.preventDefault(); setBusy(true); setMessage(""); void (clientReviewClient ? signInClientReview(email, accessCode) : otpSent ? verifyOtp(email, otp) : requestOtp(email).then((result) => { setSandboxOtp(result.sandboxOtp); setOtpSent(true); setMessage(result.sandboxOtp ? "Local sandbox mailbox: enter the displayed OTP." : "OTP sent. Check the email address you entered to continue."); })).catch((e: unknown) => setMessage(clientSafeError(e))).finally(() => setBusy(false)); }}>
            ...
            <button disabled={busy} className="h-12 w-full rounded-full bg-forest text-white font-semibold disabled:opacity-50">{busy ? "Checking…" : clientReviewClient ? "Continue" : otpSent ? "Open passport" : "Send OTP"}</button>
            {!clientReviewClient && otpSent ? <button type="button" onClick={() => { setOtpSent(false); setOtp(""); setSandboxOtp(undefined); setMessage(""); }} className="w-full text-[12px] underline">Use a different email</button> : null}
          </form>
          {message ? <p role="alert" className="mt-3 rounded-xl bg-[#fff5f5] p-3 text-[12px] text-red-700">{message}</p> : null}
          <p className="mt-4 text-[11px] leading-4 text-ink-muted">{clientReviewClient ? "This client review uses synthetic data and a temporary access code. No email service is required." : "Local sandbox codes are shown only during local development. Client review requires an approved sign-in transport."}</p>
        </section>
      </div>
    </main>
  );
