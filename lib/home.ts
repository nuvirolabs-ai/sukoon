...
export async function getHomeProjectionForUser(userId: string) {
  ...
  if (!workspace) {
    ...
    if (!hasSharedHistory) return { mode: "owner" as const, properties: [], summary: { propertyCount: 0, portfolioValuePaise: "0", outstandingPaise: "0", unreadAttention: 0, openMaintenance: 0, pendingDocumentReview: 0 }, attention: [], activity: [], education: { currentCount: (await listPublishedEducation()).length }, scope: "No property records yet. Add your first Property Passport; no sample data is created." };
    ...
    return { mode: "shared" as const, properties, summary: { propertyCount: properties.length }, attention: [], activity, education: { currentCount: (await listPublishedEducation()).length }, scope: "Only records explicitly shared with this identity are shown." };
  }
