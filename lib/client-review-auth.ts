...
export function clientReviewAuthConfigured(env: NodeJS.Dict<string> = process.env) {
  ...
  return isClientReviewAuthEnvironment(env) && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email) && code.length >= 32;
}
...
export function clientReviewAuthPlugin(): BetterAuthPlugin {
  return {
    ...
    version: "1.0.0",
    endpoints: {
      clientReviewSignIn: createAuthEndpoint("/client-review/sign-in", {
        method: "POST",
        body: bodySchema,
        metadata: { noStore: true },
      }, async (ctx) => {
        if (!isClientReviewAuthEnvironment()) throw APIError.fromStatus("NOT_FOUND");
        if (!clientReviewAuthConfigured()) throw APIError.fromStatus("SERVICE_UNAVAILABLE", { message: "Client review sign-in is not configured." });

        const email = normalizedEmail(ctx.body.email);
        const key = requestKey(email, ctx.request);
        ...
        const codeMatches = accessCodeMatches(ctx.body.accessCode, configuredCode());
