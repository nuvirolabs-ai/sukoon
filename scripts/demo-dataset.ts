...
import { assertStagingSeedEnvironment, STAGING_DATABASE_NAME } from "../lib/staging-seed-policy";
...
async function seed() {
  ...
  const docs = {};
  for (const [label, category, prop] of [
  ...
  if (taxPaid < paise(DEMO_MONEY_EXPECTATIONS.propertyTaxPayment)) {
    ...
    await recordPaymentForUser(owner.id, taxOcc.id, { amount: demoRupees(DEMO_MONEY_EXPECTATIONS.propertyTaxPayment), currency: "INR", paymentDate: iso(-1), method: "Owner-recorded demo payment", idempotencyKey: key("property-tax-payment"), receiptDocumentId: docs["Demo Property Tax Receipt — Vijay Nagar"].id, receiptDocumentVersionId: taxReceiptVersion.id, notes: "Synthetic owner-entered self-reported payment; no provider verification." });
  }
  ...
  const wp = await prisma.maintenance.findFirst({ where: { workspaceId: ws.id, propertyId: p1.id, task: "Terrace Waterproofing" } }); if (wp && docs["Demo Waterproofing Invoice — Vijay Nagar"].scanStatus === "clean") await linkMaintenanceDocumentForUser(owner.id, wp.id, { documentId: docs["Demo Waterproofing Invoice — Vijay Nagar"].id, linkType: "INVOICE" });
  ...
  const constructionDocumentLinks = [[docs["Demo Sanction Map — Super Corridor"], stages[3] ?? stage], [docs["Demo Architectural Plan — Super Corridor"], stages[4] ?? stage], [docs["Demo Structural Drawing — Super Corridor"], stage], [docs["Demo Foundation Invoice — Super Corridor"], stage]];
  ...
  for (const [email, role, user, scopes] of [[PERSONAS.coowner, "FAMILY", coowner, [{ capability: "PROPERTY_BASIC_READ" }, { capability: "BILLS_READ" }, { capability: "DOCUMENT_LIST" }, { capability: "DOCUMENT_METADATA_READ" }, { capability: "DOCUMENT_PREVIEW", documentId: docs["Demo Registry — Vijay Nagar"].id }]], [PERSONAS.lawyer, "LAWYER", lawyer, [{ capability: "PROPERTY_BASIC_READ" }, { capability: "DOCUMENT_LIST" }, { capability: "DOCUMENT_METADATA_READ" }, { capability: "DOCUMENT_PREVIEW", documentId: docs["Demo Registry — Vijay Nagar"].id }, { capability: "DOCUMENT_PREVIEW", documentId: docs["Demo Mutation — Vijay Nagar"].id }]], [PERSONAS.architect, "ARCHITECT", architect, [{ capability: "PROPERTY_BASIC_READ" }, { capability: "DOCUMENT_LIST" }, { capability: "DOCUMENT_METADATA_READ" }, { capability: "DOCUMENT_PREVIEW", documentId: docs["Demo Sanction Map — Vijay Nagar"].id }]]]) { const existing = await prisma.shareLink.findFirst({ where: { workspaceId: ws.id, propertyId: p1.id, inviteeEmail: email, revokedAt: null } }); if (!existing) { const inv = await createShareInvitationForUser(owner.id, p1.id, { inviteeEmail: email, role, scopes, expiresAt: at(180).toISOString(), note: "SYNTHETIC DEMO DATASET v1 — scoped access" }); await acceptShareInvitationForUser(user.id, email, inv.invitationToken); } }
  const architectShare = await prisma.shareLink.findFirst({ where: { workspaceId: ws.id, propertyId: p1.id, inviteeEmail: PERSONAS.architect, revokedAt: null } }); if (architectShare) { await prisma.shareLinkScope.deleteMany({ where: { shareLinkId: architectShare.id } }); await prisma.shareLinkScope.createMany({ data: [{ id: randomUUID(), shareLinkId: architectShare.id, scopeType: "PROPERTY_BASIC_READ" }, { id: randomUUID(), shareLinkId: architectShare.id, scopeType: "DOCUMENT_LIST", documentId: docs["Demo Sanction Map — Vijay Nagar"].id }, { id: randomUUID(), shareLinkId: architectShare.id, scopeType: "DOCUMENT_METADATA_READ", documentId: docs["Demo Sanction Map — Vijay Nagar"].id }, { id: randomUUID(), shareLinkId: architectShare.id, scopeType: "DOCUMENT_PREVIEW", documentId: docs["Demo Sanction Map — Vijay Nagar"].id }] }); }
  ...
  console.log(JSON.stringify(j({ status: stagingSeed ? "SEEDED_QUEUED_FOR_STAGING_WORKER" : "SEEDED", dataset: "SYNTHETIC DEMO DATASET", version: VERSION, signIn: stagingSeed ? "Use the approved staging SMTP flow; no local sandbox mailbox is used." : "http://localhost:3100/ then enter email, Send OTP, use the code shown by local sandbox, Open passport", personas: PERSONAS, properties: [p1, p2, p3], activeProject: project, purchaseCandidateId: candidate.id, ...(stagingSeed ? { database: STAGING_DATABASE_NAME, documentProcessing: "queued for the independent staging worker", storage: "private S3-compatible staging storage" } : { markerPath }) })));
}
